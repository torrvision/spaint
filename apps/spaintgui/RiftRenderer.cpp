...
RiftRenderer::RiftRenderer(const spaint::SpaintPipeline_Ptr& spaintPipeline, const std::string& title, RiftRenderingMode renderingMode)
: Renderer(spaintPipeline)
{
  // Initialise the Rift.
  ovr_Initialize();
  ...
  ITMLib::Vector2<int> depthImageSize = spaintPipeline->get_model()->get_depth_image_size();
...
void RiftRenderer::render() const
{
  ...
  {
    m_camera->set_from(CameraPoseConverter::pose_to_camera(m_spaintPipeline->get_model()->get_pose()));
  }
  ...
  m_spaintPipeline->get_raycaster()->generate_free_raycast(m_eyeImages[ovrEye_Left], m_visualisationStates[ovrEye_Left], leftPose);
  m_spaintPipeline->get_raycaster()->generate_free_raycast(m_eyeImages[ovrEye_Right], m_visualisationStates[ovrEye_Right], rightPose);
