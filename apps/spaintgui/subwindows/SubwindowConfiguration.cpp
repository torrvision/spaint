...
SubwindowConfiguration_Ptr SubwindowConfiguration::make_default(size_t subwindowCount, const Vector2i& imgSize)
{
  ...
  switch(subwindowCount)
  {
    case 1:
    {
      config->add_subwindow(Subwindow(Vector2f(0, 0), Vector2f(1, 1), VisualisationGenerator::RT_SEMANTICLAMBERTIAN, imgSize));
      break;
    }
    case 2:
    {
      const float x = 0.5f;
      config->add_subwindow(Subwindow(Vector2f(0, 0), Vector2f(x, 1), VisualisationGenerator::RT_SEMANTICLAMBERTIAN, imgSize));
      config->add_subwindow(Subwindow(Vector2f(x, 0), Vector2f(1, 1), VisualisationGenerator::RT_SEMANTICCOLOUR, imgSize));
      break;
    }
    case 3:
    {
      ...
      const float y = 0.5f;
      config->add_subwindow(Subwindow(Vector2f(0, 0), Vector2f(x, y * 2), VisualisationGenerator::RT_SEMANTICLAMBERTIAN, imgSize));
      config->add_subwindow(Subwindow(Vector2f(x, 0), Vector2f(1, y), VisualisationGenerator::RT_SEMANTICCOLOUR, imgSize));
      config->add_subwindow(Subwindow(Vector2f(x, y), Vector2f(1, y * 2), VisualisationGenerator::RT_SEMANTICPHONG, imgSize));
      break;
    }
    default:
      break;
