...
SubwindowConfiguration_Ptr SubwindowConfiguration::make_default(size_t subwindowCount, const Vector2i& imgSize, const std::string& pipelineType)
{
  ...
  switch(subwindowCount)
  {
    ...
    case 7:
    {
      ...
      config->add_subwindow(Subwindow(Vector2f(0, y), Vector2f(x, 1 - y), "Remote1", VisualisationGenerator::VT_SCENE_COLOUR, imgSize));
      config->add_subwindow(Subwindow(Vector2f(0, 1 - y), Vector2f(x, 1), "Remote2", VisualisationGenerator::VT_SCENE_COLOUR, imgSize));
      config->add_subwindow(Subwindow(Vector2f(1 - x, 0), Vector2f(1, y), "Remote3", VisualisationGenerator::VT_SCENE_COLOUR, imgSize));
      config->add_subwindow(Subwindow(Vector2f(1 - x, y), Vector2f(1, 1 - y), "Remote4", VisualisationGenerator::VT_SCENE_COLOUR, imgSize));
      config->add_subwindow(Subwindow(Vector2f(1 - x, 1 - y), Vector2f(1, 1), "Remote5", VisualisationGenerator::VT_SCENE_COLOUR, imgSize));
      break;
    }
    default:
      break;
