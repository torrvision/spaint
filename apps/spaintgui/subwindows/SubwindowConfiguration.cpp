...
SubwindowConfiguration_Ptr SubwindowConfiguration::make_default(size_t subwindowCount, const Vector2i& imgSize, const std::string& pipelineType)
{
  ...
  switch(subwindowCount)
  {
    ...
    case 7:
    {
      ...
      config->add_subwindow(Subwindow(Vector2f(1 - x, 0), Vector2f(1, y), worldSceneID, VisualisationGenerator::VT_SCENE_COLOUR, imgSize));
      config->add_subwindow(Subwindow(Vector2f(1 - x, y), Vector2f(1, 1 - y), worldSceneID, VisualisationGenerator::VT_SCENE_COLOUR, imgSize));
      config->add_subwindow(Subwindow(Vector2f(1 - x, 1 - y), Vector2f(1, 1), worldSceneID, VisualisationGenerator::VT_SCENE_COLOUR, imgSize));
      break;
    }
    default:
      break;
