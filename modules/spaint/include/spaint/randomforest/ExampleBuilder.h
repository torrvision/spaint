...
public:
   ...
   * \brief TODO
   */
  static std::vector<Descriptor_CPtr> make_descriptors(const ORUtils::MemoryBlock<float>& featuresMB, size_t sampledVoxelCount, size_t featureCount)
  {
    // Make the descriptors.
    featuresMB.UpdateHostFromDevice();
    const float *features = featuresMB.GetData(MEMORYDEVICE_CPU);
    std::vector<Descriptor_CPtr> descriptors(sampledVoxelCount);
    ...
    for(size_t i = 0; i < sampledVoxelCount; ++i)
    {
      ...
      descriptors[descriptorIndex++].reset(new Descriptor(featuresForDescriptor, featuresForDescriptor + featureCount));
    }
