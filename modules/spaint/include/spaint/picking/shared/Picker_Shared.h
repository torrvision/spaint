 ...
 */
_CPU_AND_GPU_CODE_
inline bool get_pick_point(int x, int y, int width, const Vector4f *pointImage, Vector3s& pickPoint)
{
  Vector4f p = pointImage[y * width + x];
  pickPoint = Vector3s(static_cast<short>(p.x), static_cast<short>(p.y), static_cast<short>(p.z));
  return p.w > 0;
}
