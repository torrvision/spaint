...
void LabelPropagator_CUDA::calculate_normals(const ITMFloat4Image *raycastResult, const Scene *scene) const
{
...
void LabelPropagator_CUDA::perform_propagation(SpaintVoxel::Label label, const ITMFloat4Image *raycastResult, Scene *scene) const
{
  const int raycastResultSize = static_cast<int>(raycastResult->dataSize);
